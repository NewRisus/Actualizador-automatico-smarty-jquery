...
if($yourScript === 'phpost' || $yourScript === 'newrisus') {
	include "{$raiz}/header.php";
} elseif($yourScript === 'newrisus2') {
	include "{$raiz}/lib/header.php";
}

# Incluimos el archivo de smarty
if($yourScript == 'newrisus' || $yourScript == 'newrisus2'):
...
<?php if(empty($act)): ?>
   <div class="col-xxl-8 col-xl-8 col-lg-8 col-md-10 col-sm-12 col-12 offset-xxl-2 offset-xl-2 offset-lg-2 offset-md-1 offset-sm-0">
	   ...
	   <div class="row rows-3">
			<div class="col">
				<div id="smarty36"<?php if('3.1.36' > $version_now): ?> onclick="location.href=global.url + '/<?php echo $yourScript; ?>/smarty/3_1_36'"<?php endif; ?> class="box rounded shadow d-flex justify-content-center align-items-center flex-column py-5">
					<i class="bi bi-cloud-upload"></i>
					<span class="text-white text-uppercase">Smarty 3.1.36</span>
				</div>
			</div>
			<div class="col">
				<div id="smarty37"<?php if('3.1.37' > $version_now): ?> onclick="location.href=global.url + '/<?php echo $yourScript; ?>/smarty/3_1_37'"<?php endif; ?> class="box rounded shadow d-flex justify-content-center align-items-center flex-column py-5">
					<i class="bi bi-cloud-upload"></i>
					<span class="text-white text-uppercase">Smarty 3.1.37</span>
				</div>
			</div>
			<div class="col">
				<div id="smarty39"<?php if('3.1.39' > $version_now): ?> onclick="location.href=global.url + '/<?php echo $yourScript; ?>/smarty/3_1_39'"<?php endif; ?> class="box rounded shadow d-flex justify-content-center align-items-center flex-column py-5">
					<i class="bi bi-cloud-upload"></i>
	   	...
	   	<?php else: ?>
	   		...
	   		<div class="border-top border-light mt-4 py-2">
	            <form method="POST" id="smarty_install">
	               ...
	               <input type="hidden" name="script" value="<?php echo ($yourScript == 'phpost') ? 1 : ($yourScript == 'newrisus' ? 2 : 3); ?>">
